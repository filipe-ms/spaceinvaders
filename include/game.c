...
void UpdateWave() {
	...
	else if (active_wave == BOSS) BossWave();
}
...
void UpdateGame(void)
{
    ...
    if (!pause)
    {
        ...
        if (level_up_flag) {
            ...
            if (CheckBossCollisionWithPlayer(player)) ChangeScene(GAME_OVER);
        }
    }
}
...
void DrawGame(void)
{
	...
	else if (active_wave == BOSS) DrawText("BOSS", SCREEN_WIDTH / 2 - MeasureText("BOSS", 40) / 2, SCREEN_HEIGHT / 2 - 100, 40, Fade(WHITE, wave_message_alpha));
